const API_BASE_URL = 'https://gatherly-app-592179280005.us-central1.run.app/api';

export async function createChat(creatorId: string, chatName: string, agenda: string) {
  const response = await fetch(`${API_BASE_URL}/chats/create`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({ creator_id: creatorId, chat_name: chatName, agenda }),
  });
